...
class Command(BaseCommand):
    ...
    def _create_folders(
        ...
        specs = [
            ...
            ("medicine_cases", "Medicine Demo Cases", departments["legal"], "contracts"),
            ("business_cases", "Business Demo Cases", departments["finance"], "budgets"),
            ("quasi_gov_cases", "Quasi-Government Demo Cases", departments["executive"], "orders"),
    ...
    def _document_specs(self) -> list[DemoDocumentSpec]:
        return [
            ...
            DemoDocumentSpec(
                key="pilot_addendum",
                title="Additional Agreement 1 for DMS Pilot Services",
                filename="demo_pilot_addendum.pdf",
                file_bytes=build_simple_pdf(
                    "Additional Agreement 1",
                    [
                        "Synthetic addendum extending the pilot support period.",
                    ],
                ),
                department="legal",
                folder="ocr_project",
                doc_type="addendum",
                description="Synthetic additional agreement tied to the pilot contract.",
                extracted_text="Additional agreement 1 to the DMS pilot services contract. The parties extend support for the controlled pilot period without changing confidential or real customer data.",
                doc_date=date(2026, 2, 25),
                author="Legal Department",
                retention_category="Contract addendum",
                retention_until=date(2036, 2, 25),
            ),
            DemoDocumentSpec(
                key="medicine_supply_contract",
                title="Medicine Demo: Clinic Supply Contract",
                filename="demo_medicine_supply_contract.pdf",
                file_bytes=build_simple_pdf(
                    "Clinic Supply Contract",
                    [
                        "Synthetic medical vertical contract without patient data.",
                    ],
                ),
                department="legal",
                folder="medicine_cases",
                doc_type="contract",
                description="Synthetic healthcare supply document used to show sector-neutral document search.",
                extracted_text="Medicine demo supply contract. Demo Clinic receives sterile consumables from MedSupply Demo LLP. The document contains no patient data, no real BIN/IIN, and no real medical records.",
                doc_date=date(2026, 4, 12),
                author="Legal Department",
                retention_category="Sector demo contract",
                retention_until=date(2036, 4, 12),
            ),
            DemoDocumentSpec(
                key="business_services_invoice",
                title="Business Demo: Consulting Services Invoice",
                filename="demo_business_services_invoice.pdf",
                file_bytes=build_simple_pdf(
                    "Consulting Services Invoice",
                    [
                        "Synthetic B2B invoice for business vertical demo.",
                    ],
                ),
                department="finance",
                folder="business_cases",
                doc_type="invoice",
                description="Synthetic business invoice for semantic search and B2B chain demonstration.",
                extracted_text="Business demo invoice. Atlas Business Demo LLP issues an invoice for consulting services connected to a synthetic contract chain. Amount 950000 KZT.",
                doc_date=date(2026, 4, 18),
                author="Finance Department",
                retention_category="Sector demo invoice",
                retention_until=date(2031, 4, 18),
            ),
            DemoDocumentSpec(
                key="quasi_gov_status_report",
                title="Quasi-Government Demo: Digital Archive Status Report",
                filename="demo_quasi_gov_status_report.pdf",
                file_bytes=build_simple_pdf(
                    "Digital Archive Status Report",
                    [
                        "Synthetic quasi-government reporting example.",
                    ],
                ),
                department="executive",
                folder="quasi_gov_cases",
                doc_type="report",
                description="Synthetic quasi-government style report for mixed-language search demonstration.",
                extracted_text="Quasi-government demo report. Demo QazService summarizes the digital archive pilot, document routing, audit events, and controlled evidence export. Есеп цифрлық мұрағат пилотын сипаттайды.",
                doc_date=date(2026, 4, 20),
                author="Executive Office",
                retention_category="Sector demo report",
                retention_until=date(2031, 4, 20),
                language=Document.Language.KK,
            ),
            DemoDocumentSpec(
    ...
    def _create_related_documents(self, documents: dict[str, Document]) -> None:
        relations = [
            ...
            ("pilot_addendum", "ocr_contract", DocumentRelation.RelationType.CONTRACT_TO_ADDITIONAL_AGREEMENT, "0.92"),
            ("business_services_invoice", "ocr_contract", DocumentRelation.RelationType.SAME_PROJECT, "0.78"),
            ("medicine_supply_contract", "ocr_contract", DocumentRelation.RelationType.SAME_PROJECT, "0.76"),
            ("quasi_gov_status_report", "launch_order", DocumentRelation.RelationType.REFERENCES, "0.82"),
